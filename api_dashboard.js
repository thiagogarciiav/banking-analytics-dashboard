...
// Node.js 20+ | API + arquivos estáticos para o dashboard
...
// Fallback local (Docker injeta DATABASE_URL)
if (!process.env.DATABASE_URL) {
  process.env.DATABASE_URL = 'postgresql://banco:banco123@localhost:5432/banco_digital';
}
...
// ---------- Arquivos estáticos ----------
app.use(express.static(__dirname));           // css, js, etc.
...
function filters(query) {
  ...
  if (query.mes) {
    add("date_trunc('month', t.data_hora)::date = ?::date", `${query.mes}-01`);
  }
...
app.get('/api/dashboard', async (req, res, next) => {
  try {
    ...
    const sql = `
      WITH base AS (
        SELECT t.*, c.nome, c.cidade, c.uf, c.data_nascimento
        FROM transacoes t
      ...
      SELECT json_build_object(
        'kpis', (
          SELECT json_build_object(
            ...
            'transacoes', COUNT(*),
            'volume_aprovado',
              COALESCE(ROUND(SUM(valor) FILTER (WHERE status_transacao = 'Aprovada'), 2), 0),
            'ticket_medio',
              COALESCE(ROUND(AVG(valor) FILTER (WHERE status_transacao = 'Aprovada'), 2), 0),
            'taxa_aprovacao',
              COALESCE(ROUND(
                100.0 * COUNT(*) FILTER (WHERE status_transacao = 'Aprovada')
                / NULLIF(COUNT(*), 0), 1), 0)
          ) FROM base
        ),
        'volume_por_tipo', (
          ...
          FROM (
            SELECT tipo, ROUND(SUM(valor), 2) AS volume_aprovado
            FROM base WHERE status_transacao = 'Aprovada'
            GROUP BY tipo
          ) x
        ),
        'volume_por_estado', (
          SELECT COALESCE(json_agg(x ORDER BY x.volume_aprovado DESC), '[]')
          FROM (
            SELECT uf, ROUND(SUM(valor), 2) AS volume_aprovado
            FROM base WHERE status_transacao = 'Aprovada'
            GROUP BY uf
          ) x
        ),
        'serie_diaria', (
          ...
          FROM (
            SELECT data_hora::date AS dia, ROUND(SUM(valor), 2) AS volume_aprovado
            FROM base WHERE status_transacao = 'Aprovada'
            GROUP BY 1
          ) x
        ),
